...
The Config page also has a separate Settings panel for gameplay preferences that are not app/network configuration. Target State alerts use the db-backed localStorage data-store key `alerts.targetState`; the frontend writes it through `dataStoreService.js` and the backend reads the same SQLite `settings` value when `/api/status` builds active/pending current-system toasts. The default target state is `Infrastructure Failure`.
